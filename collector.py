...
GEO = ""                 # '' = 전세계(Worldwide)
SLEEP_BETWEEN = 6.0      # 키워드 호출 사이 대기(초). 429를 줄이려고 4→6으로 늘림.
WITH_RELATED = True      # 연관어(rising)+감성 수집. 호출이 늘어 429 위험↑ (끄려면 False)
...
def collect_live() -> tuple[list[dict], list[dict]]:
    ...
    pytrends = TrendReq(
        hl="en-US", tz=0,
        timeout=(10, 25),
        retries=4, backoff_factor=0.8,    # 429 만나면 더 끈질기게 + 더 오래 쉬며 재시도
...
def main() -> int:
    ...
    # (1) 급등 순위 — 변동성 축 (score 내림차순). HOT 뱃지는 이 순위 기준.
